...
To ensure robust evaluation, classical models, deep acoustic models (ECAPA-TDNN), and the proposed DFAT pipeline are evaluated using a strict, cryptographically verified speaker-independent protocol.
...
To guarantee full reproducibility and structural integrity, this research enforces a strict chain of provenance. The exact speaker-to-split mappings are cryptographically locked in \texttt{split\_manifest.json} via SHA-256 checksums. Model checkpoints and metadata (including hyperparameter tuning results) are dynamically verified against this checksum at evaluation time. All tabular and visual results presented in this report (via \texttt{benchmark\_results\_gpu.json}) are programmatically regenerated by \texttt{benchmark\_methods\_gpu.py} to ensure zero documentation drift.
...
\end{tabular}
%% END_ABLATION_TABLE
\caption{DFAT scaling ablation study under finalized Speaker-Independent Protocol.}
\label{tab:ablation}
\end{table}

\textbf{Interpretation}: Earlier experimental phases of this research reported an inflated Early Fusion score of 0.5238. However, that result was a product of an experimental configuration that predated the strict, cryptographically verified speaker-independent protocol. Under the finalized protocol, both scaling configurations yield equivalent performance bounded near $\sim$0.49. We deploy Phase 1 (Raw Concat) as the final model due to its simplicity and marginally higher wF1.
...
The classical MFCC+RF pipeline and the proposed DFAT pipeline were evaluated across 5 random seeds to quantify variance. However, ECAPA-TDNN was evaluated with a single seed due to its monolithic training constraints. This means its reported metrics lack variance estimates, and its absolute stability margin relative to DFAT remains unknown. Future work should prioritize multi-seed evaluation for deep baseline models.
