...
& \texttt{Whisper-small} & ASR transcript quality. & Best quality-latency inflection point; \texttt{tiny} degrades text quality catastrophically (ablation: $-0.14$ wF1). \\
...
\textbf{Interpretation}: The WavLM acoustic branch acts as the primary discriminative anchor. Removing the audio branch (Linguistic-only) drops performance dramatically, suggesting that ASR text alone is insufficient to determine emotion. Simple uncalibrated fusion (Early Fusion at 0.3995 or stream-level Late Fusion at 0.3408) can actually underperform the acoustic-only baseline (0.4329), indicating that the linguistic stream acts best as a supplementary signal, not an independent equal. Downgrading Whisper-small to Whisper-tiny drops linguistic performance precipitously, confirming that fusion success is highly dependent on ASR transcript quality.
...
DFAT's linguistic stream is strictly upper-bounded by Whisper's transcription accuracy. Under extreme emotional prosody (shouting, whispering, crying), Whisper's word error rate increases substantially. This creates a cascading failure: poor transcripts produce out-of-distribution PhoBERT embeddings, which can actively harm the fusion rather than help it. Our ablation confirms this risk: the \texttt{Whisper-tiny} variant degrades fusion performance below the acoustic-only baseline.
