...
Four additional secondary baselines are included for comprehensive comparison.
...
margins. Ablation studies confirm that both acoustic and linguistic streams
contribute to DFAT's superiority, with the linguistic stream providing the
largest gains for acoustically ambiguous classes.
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{Seed Control}: Stochastic processes in classical ML models are
    controlled via 5 fixed random seeds (e.g., 42, 43, 44, 45, 46) to evaluate
    stability. Deep learning architectures utilize deterministic singular runs.
\end{itemize}
