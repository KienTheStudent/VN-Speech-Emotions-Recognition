...
\textbf{Summary of Error Modalities}:
In our evaluation, misclassifications broadly stem from three sources:
\begin{itemize}
    \item \textbf{Acoustic Ambiguity}: Errors like Happy $\rightarrow$ Neutral occur because conversational speech lacks the exaggerated pitch variations found in acted corpora.
    \item \textbf{ASR Noise}: Errors where Angry is misclassified arise when high-energy speech degrades Whisper's transcription, introducing linguistic noise that misleads the text branch.
    \item \textbf{Class Overlap}: Sad and Neutral states intrinsically overlap in both low acoustic energy and subdued lexical content, making them challenging to separate regardless of the modality used.
\end{itemize}
...
\end{itemize}
\textbf{Conclusion}: The $+0.35$ gain in wF1 offered by DFAT over classical baselines comes at a severe computational cost. It appears exclusively suited for cloud-hosted environments where high accuracy justifies the increased inference budget.
