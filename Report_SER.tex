...
I would like to express my sincere gratitude to my advisors, professors, and peers for their invaluable guidance and support throughout this thesis.
...
This thesis provides several key contributions:
\begin{itemize}
    \item A systematic, leak-free benchmark comparing classical ML with modern deep learning for Vietnamese SER.
    \item The proposal of DFAT, an ASR-assisted audio-linguistic fusion pipeline that enhances emotion classification by leveraging text generated from speech, explicitly acknowledging this as pseudo-multimodality rather than independent text input.
    \item A transparent evaluation protocol utilizing cryptographic checksums to enforce a strict speaker-independent split, exposing the generalization limits of classical methods.
\end{itemize}

\chapter{Objectives}
This work presents a systematic evaluation of multiple SER approaches on Vietnamese speech, comparing feature-based classical methods with contemporary deep learning techniques and a proposed ASR-assisted audio-linguistic fusion system. The primary objectives are to address the following research questions:
...
To ensure the robustness of our findings, classical stochastic pipelines (RF, XGBoost) are trained and evaluated over 5 random seeds to report mean $\pm$ standard deviation. Deterministic models (LogReg, SVC-RBF) are evaluated once. For the deep models (ECAPA-TDNN and DFAT), performance metrics are derived from their pre-calculated evaluation metadata over a standardized run, ensuring reproducibility without requiring full multi-seed GPU reruns during the benchmark script.
...
\subsection{RQ1: Classical Baselines and the Impact of Speaker-Independent Splits}
Under our strict speaker-independent protocol, the classical MFCC+RF pipeline achieves substantially lower performance compared to historical benchmarks on ViSEC. Previous benchmarks often utilized random stratified sampling, which inherently allows speaker identity leakage between the train and test sets. In such scenarios, classical ML models inadvertently memorize speaker-specific acoustic features rather than generalized emotion patterns. By enforcing a strict cryptographic split where test speakers are completely unseen during training, we expose the genuine generalization ceiling of handcrafted MFCC features. The deep learning ECAPA-TDNN baseline, designed explicitly for generalized representation learning, experiences a far smaller drop and significantly outperforms the classical pipeline, proving its robustness across diverse vocal tracts.

\subsection{RQ2: ASR-Assisted Fusion -- Where DFAT Wins and Loses}
DFAT Late Fusion Ensemble demonstrates substantial improvements over audio-only baselines, but the gains are not uniform across all emotion classes. 
\textbf{Where DFAT Wins:} The improvement is most pronounced for the \textit{Neutral} class, which is acoustically ambiguous. Neutral speech often lacks distinct prosodic markers, causing audio-only models like ECAPA-TDNN to struggle. By incorporating ASR-derived linguistic embeddings (PhoBERT), DFAT leverages lexical cues to disambiguate Neutral utterances successfully, lifting its F1-score significantly.
\textbf{Where DFAT Loses:} DFAT struggles when transcription noise overwhelms the linguistic stream. Rapid, highly emotional outbursts (especially \textit{Happy} or \textit{Angry}) often cause Whisper to drop or misinterpret words. In these instances, the corrupted text forces the PhoBERT embedding into out-of-distribution spaces, causing the fusion system to misclassify the utterance.

\subsection{RQ3: Component Contributions via Ablation}
The ablation study confirms that both streams provide complementary signals. While the WavLM acoustic stream provides the primary discriminative power, the linguistic stream acts as a critical regularizer for acoustically overlapping classes. Furthermore, substituting Whisper-small with Whisper-tiny causes performance to drop precipitously, confirming the entire pipeline's high sensitivity to the underlying ASR quality. Early fusion (concatenation) underperforms compared to Late Fusion because it forces the classifier to directly model the noise in the Whisper transcripts, whereas Late Fusion allows the ensemble to intelligently weight the stream probabilities to marginalize transcription errors.

\subsection{RQ4: Compute Trade-Off (Feature Extraction vs. Classifier Latency)}
When evaluating cost-efficiency, we must decompose latency into two axes: feature extraction and classifier execution.
\begin{itemize}
    \item \textbf{Feature Extraction Latency:} Classical methods (MFCC) require negligible CPU time ($\ll 1$ ms/sample). In contrast, DFAT requires sequential passes through three large models (WavLM 94M parameters, Whisper 244M parameters, and PhoBERT 135M parameters), dominating the end-to-end inference time and strictly requiring a GPU.
    \item \textbf{Classifier Latency:} Once features are extracted, the Random Forest and XGBoost classifiers execute in sub-milliseconds. 
\end{itemize}
Therefore, while MFCC+RF offers the best cost-efficiency ratio for edge deployment, DFAT represents a heavy-compute, high-accuracy paradigm strictly suited for cloud environments where transcription latency can be absorbed.

\section{Error Analysis}

Detailed analysis of the confusion matrices highlights remaining structural challenges in emotion separation. We specifically analyze two major confusion pairs from the DFAT results:

\begin{itemize}
    \item \textbf{Happy $\rightarrow$ Neutral (31 cases)}: In Vietnamese, conversational ``Happy'' speech often lacks the dramatic pitch elevation seen in staged acting. When speakers express mild happiness, the acoustic profile flattens, resembling neutrality. Without strong lexical markers (e.g., explicit words of joy), the ASR-derived text also reads as neutral. Whisper may further transcribe these rapid conversational segments ambiguously, leading both the acoustic and linguistic streams to default to the majority ``Neutral'' class.
    \item \textbf{Sad $\rightarrow$ Neutral (20 cases)}: Both states exhibit low acoustic energy, narrow pitch ranges, and slower speaking rates. In the linguistic stream, PhoBERT embeddings for short, low-affect speech segments tend to cluster tightly in the semantic space. When a user sighs or speaks briefly, Whisper often outputs truncated text, stripping away the phonetic nuances of sadness and causing the model to misclassify it as Neutral.
\end{itemize}

\subsection{ASR Error Propagation}
...
To properly frame our findings, we identify the following key threats to validity:
\begin{table}[h]
\centering
\begin{tabular}{lp{10cm}}
\toprule
\textbf{Threat} & \textbf{Description \& Mitigation} \\
\midrule
\textbf{Speaker Imbalance} & Speaker 0 contributes 42\% of training samples. We mitigated this by enforcing a strict speaker-independent split, ensuring test set performance reflects genuine generalization rather than overfitting to Speaker 0. \\
\textbf{ASR Noise} & Whisper's transcription errors introduce significant noise into the linguistic stream, directly propagating to the classifier. \\
\textbf{Pseudo-Multimodality} & The text stream is derived purely from the audio via ASR, meaning it is not a true independent modality (unlike distinct video or written text feeds). It represents inferred phonetic content, not ground truth text. \\
\textbf{Dataset Scale} & ViSEC contains 5,280 samples, which is moderate compared to modern foundational deep learning scales. \\
\bottomrule
\end{tabular}
\caption{Key threats to validity in this study.}
\label{tab:threats}
\end{table}
...
While DFAT establishes a strong baseline for ASR-assisted SER on Vietnamese, several avenues remain for future exploration. The high latency of sequential processing suggests that end-to-end multi-task learning (simultaneously predicting transcription and emotion) could provide similar benefits at a fraction of the computational cost. Additionally, extending the benchmark to include visual modalities (e.g., facial expressions) would yield a truly multimodal system, overcoming the limitations of pseudo-multimodality. Finally, expanding the dataset with more diverse, spontaneous conversational data will be critical to addressing speaker imbalance and improving real-world generalization.

\appendix
\chapter{Supplementary Material}
The source code, detailed hyperparameter configurations, and reproduction scripts for all experiments are available in the associated GitHub repository. The cryptographic verification module for the dataset split is provided in \texttt{generate\_splits.py}.
