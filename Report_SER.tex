...
\section{Discussion}
The findings reveal the true difficulty of Vietnamese SER. Classical methods like MFCC fail dramatically in a speaker-independent setting because they memorize speaker timbre instead of generalized emotion semantics. The ECAPA-TDNN generalized its representation significantly better. Ultimately, the superior performance of ECAPA-TDNN demonstrates that maintaining continuous, high-resolution acoustic features is far more critical for emotion recognition than extracting discrete semantic tokens. While humans instinctively rely on semantic context when a tone is ambiguous, computationally forcing this via pseudo-multimodality (ASR text) creates a severe bottleneck bound by the ASR system's word error rate under noisy emotional conditions.
