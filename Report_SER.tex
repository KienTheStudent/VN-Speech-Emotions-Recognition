...
To ensure robust evaluation, classical models and the proposed DFAT pipeline are evaluated over 5 random seeds
to report mean $\pm$ standard deviation. Due to its monolithic training constraint, ECAPA-TDNN is evaluated
on a single seed.

Driven by Weighted F1 as the primary anchor metric, results on the held-out
test set demonstrate that ECAPA-TDNN achieves the highest performance, outperforming
the proposed DFAT pipeline. Analysis reveals that while linguistic fusion provides supplementary
cues, the explicit ASR bottleneck loses critical acoustic paralinguistics (e.g., tone, cadence),
whereas end-to-end acoustic modeling in ECAPA-TDNN captures and retains them more effectively.
These findings highlight the challenges of using ASR pipelines for affective computing 
without tight, low-level acoustic integration.
\end{abstract}
...
\midrule
\quad \textbf{ECAPA-TDNN (simplified implementation)} & 0.6137 & — & 0.6131 & 0.6098 \\
\quad \textbf{DFAT Late Fusion} & 0.5087 & 0.0117 & 0.5049 & 0.5092 \\
...
\textbf{Interpretation}: Under the rigorous speaker-independent evaluation protocol, ECAPA-TDNN achieves the highest performance with a wF1 of 0.6137, significantly outperforming the proposed DFAT Late Fusion pipeline (wF1 = 0.5087). While DFAT outperforms the classical MFCC+RandomForest baseline, it falls short of the end-to-end deep acoustic model. The standard deviation ($\sigma=0.0117$) for DFAT across 5 seeds indicates stable convergence, but its absolute performance ceiling is restricted by the ASR component.
...
\textbf{What happened}: The linguistic stream struggled to compensate for the emotional signal lost during the transcription process, causing DFAT to underperform ECAPA-TDNN in every single class.
...
\begin{enumerate}
    ...
    \item \textbf{The supremacy of end-to-end acoustics}: The ECAPA-TDNN model achieved the highest performance (0.6137 wF1) on the test set, substantially outperforming the multimodal DFAT pipeline (0.5087 wF1).
    \item \textbf{The vulnerability of ASR pipelines}: Through ablation and failure analysis, we established that while ASR-derived text offers supplementary semantic context, the explicit transcription bottleneck destroys critical acoustic paralinguistics. The cascading ASR noise renders forced multimodal fusion inferior to pure acoustic representation learning in Vietnamese SER.
\end{enumerate}
